...
/* ─── Single project card (pure UI, no server deps) ─────── */

const Project = ({ project }: { project: any }) => {
  const { _id, title, slug, date, coverImage } = project;
    ...
    <article className="flex flex-col rounded-lg border border-gray-700 bg-gray-800 shadow-sm overflow-hidden h-full hover:shadow-md transition-all duration-200 hover:border-cyan-400">
      {coverImage && (
        <div className="relative w-full h-48 overflow-hidden">
          <Image
            src={src}
            alt={coverImage.alt || title || "Project cover image"}
            fill
...
/* ─── Client viewer — handles show more/less ─────────────── */

const INITIAL_COUNT = 3;

export function ProjectViewer({ data }: { data: any[] }) {
